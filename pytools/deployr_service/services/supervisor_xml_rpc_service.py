...
class SupervisorXmlRpcService(DeployrBase):
    ...
    def __init__(self, config):
        super(SupervisorXmlRpcService, self).__init__(config)
    ...
    def start(self, app_name):
        ...
        try:
            ...
            self.loggr.error('Could not start process \'{}\'! Error: {}'.format(app_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def stop(self, app_name):
        ...
        try:
            ...
            self.loggr.error('Could not stop process \'{}\'! Error: {}'.format(app_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def restart(self, app_name):
        ...
        if (stop_state + start_state) > RETURNCODE.OS_SUCCESS:
            self.loggr.error('Could not restart process \'{}\'!'.format(app_name))
            return RETURNCODE.OS_ERROR
    ...
    def add_group(self, group_name):
        ...
        try:
            ...
            self.loggr.error('Could not add process group \'{}\'! Error: {}'.format(group_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def status(self):
        ...
        """
        self.loggr.debug('SUPERVISOR XML-RPC({}): Requesting status'.format(self.supervisord_xml_rpc_server))
        try:
            response = self.server.supervisor.getState()
            if 'statename' in response:
                if response['statename'] == 'RUNNING':
                    self.loggr.debug('SUPERVISOR XML-RPC({}): Status \'{}\''.format(response['statename']))
                    return RETURNCODE.OS_SUCCESS
            return RETURNCODE.OS_ERROR
        except xmlrpclib.Fault, e:
            self.loggr.error('Could not get status! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def get_all_process_info(self):
        """
            Request status of given application
        """
        self.loggr.debug('SUPERVISOR XML-RPC({}): Requesting all process info'.format(self.supervisord_xml_rpc_server))
        try:
            return self.server.supervisor.getAllProcessInfo()
        except xmlrpclib.Fault, e:
            self.loggr.error('Could not get all process info! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def get_process_info(self, app_name):
        ...
        try:
            ...
            self.loggr.error('Could not get the process info for: {}! Error: {}'.format(app_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def remove_group(self, group_name):
        ...
        try:
            ...
            self.loggr.error('Could not remove process group \'{}\'! Error: {}'.format(group_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def get_all_config_info(self):
        """
            Remove application from supervisor_api context
        """
        self.loggr.debug('SUPERVISOR XML-RPC({}): Requesting all config info'.format(self.supervisord_xml_rpc_server))
        try:
            return self.server.supervisor.getAllConfigInfo()
        except xmlrpclib.Fault, e:
            self.loggr.error('Could not get all config info! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def get_config_info(self, app_name):
        ...
        try:
            ...
            self.loggr.error('Could not get config info for API: {}! Error: {}'.format(app_name, e))
            return RETURNCODE.OS_ERROR
        except Exception, e:
            self.loggr.error('Unknown error! Call the administrator! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
    ...
    def help_method(self, method_name):
        ...
        try:
            ...
            self.loggr.warning('Received no result on given method! Error: {}'.format(e))
            return RETURNCODE.OS_ERROR
        return RETURNCODE.OS_SUCCESS
